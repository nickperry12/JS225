...
// 2) The context object, and returns a new function that's hard-bound to
//    the passed in context object.
function myBind(func, obj, ...boundArgs) {
  return function(...args) {
    return func.apply(obj, boundArgs.concat(args));
  }
}
...
function logName(greeting) {
  console.log(greeting + ', ' + this.firstName + ' ' + this.lastName);
}

let logFullName = myBind(logName, person, 'hello');
logFullName();